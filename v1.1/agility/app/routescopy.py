...
def get_proj_name(project_id:int):
    proj_name = db.engine.execute("select project.ProjName from project where project.project_id = "+project_id)
...
@app.route('/del_project/<project_id>')
@login_required
...
def create_sprint_endpoint():
    form = SprintForm()
    if form.validate_on_submit():
        sprint = Sprint(start_date=datetime.datetime.utcnow(), end_date=datetime.datetime.utcnow() + datetime.timedelta(days=14)) # add 2 weeks to same day its generated
...
def team_endpoint(project_id):
    ...
    return render_template('team.html', title="Team", member_ids=member_ids, getUsername=getUsername, getEmail=getEmail)
...
@app.route('/sprint/<spring_id>/<project_id>')
@login_required
def sprint_endpoint(sprint_id, project_id):
    ...
    return render_template('Sprint.html', title="Sprint Page", project_id=project_id, todo=todo, inprogress=in_progress, done=done, prod_back_ids=prod_back_ids, get_title=get_title, get_difficulty=get_difficulty)
